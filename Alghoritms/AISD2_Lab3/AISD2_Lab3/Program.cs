        ...
        private static void TestReverse()
        {
            ...
            for (int i = 0; i < ReverseTestSize; ++i)
            {
                ...
                try
                {
                    ...
                    if (cr > 1.5 * cgg + g[i].VerticesCount)
                    {
        ...
        private static void TestBipartite()
        {
            ...
            for (int i = 0; i < BipartiteTestSize; ++i)
            {
                ...
                try
                {
                    ...
                    if (cr > 1.5 * cgg[i] + g[i].VerticesCount)
                    {
            ...
            for (int v = 0; v < t.VerticesCount; ++v)
                foreach (Edge e in t.OutEdges(v))
                    if (e.Weight != g.GetEdgeWeight(e.From, e.To))
                        return false;
            int gcc, tcc;
            g.GeneralSearchAll<EdgesStack>(null, null, null, out gcc);
            t.GeneralSearchAll<EdgesStack>(null, null, null, out tcc);
            return gcc == tcc;
        }
